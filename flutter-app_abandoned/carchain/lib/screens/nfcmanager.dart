...
class _NfcManagerState extends State<NfcManager> with SingleTickerProviderStateMixin {
                    ...
                    RaisedButton(
                      onPressed: () async {
                        try {
                          ...
                          } else if (tag.type == NFCTagType.mifare_ultralight || tag.type == NFCTagType.mifare_classic) {
                ...
                Row(
                  ...
                  children: <Widget>[
                      ...
                      onPressed: () async {
                        if (_message.length != 0) {
                          try {
                            ...
                            if (tag.type == NFCTagType.mifare_ultralight || tag.type == NFCTagType.mifare_classic) {
                    ...
                    RaisedButton(
                      onPressed: () {
                        setState(() {
                          _message.add(NDEFRawRecord("", "", "", ndef.TypeNameFormat.empty));
                        });
                      },
                    ...
                    (index) => GestureDetector(
                      ...
                      onTap: () async {
                        final result = await Navigator.push(context, MaterialPageRoute(builder: (context) {
                          return NDEFRecordSetting(record: _message[index]);
                        }));
                        if (result != null) {
...
class _NDEFRecordSetting extends State<NDEFRecordSetting> {
  ...
  @override
  initState() {
    _identifierController = new TextEditingController.fromValue(TextEditingValue(text: widget.record.identifier));
    _payloadController = new TextEditingController.fromValue(TextEditingValue(text: widget.record.payload));
    _typeController = new TextEditingController.fromValue(TextEditingValue(text: widget.record.type));
                  ...
                  onPressed: () {
                    if ((_formKey.currentState as FormState).validate()) {
                      Navigator.pop(
                          context,
                          NDEFRawRecord(
                              (_identifierController.text == null ? "" : _identifierController.text),
                              (_payloadController.text == null ? "" : _payloadController.text),
